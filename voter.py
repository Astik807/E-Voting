...
def establish_connection():
    ...
    password = "krish@1411"  # replace with your MySQL password
    database = "voterList"  # replace with your database name

    client_socket = mysql.connector.connect(
        host=host,
        user=user,
        password=password,
        database=database
...
# def log_server(root,frame1,client_socket,voter_ID,password):
    ...
    if(message=="Authenticate"):
        ...
        failed_return(root,frame1,client_socket,message)
def log_server(root,frame1,voter_ID,password):
    ...
    mycursor.execute("SELECT * FROM voters WHERE voter_ID=%s AND password=%s", (voter_ID, password))
    result = mycursor.fetchone()
    if result:
        votingPg(root, frame1, voter_ID)
    else:
        message = "Invalid Voter ID or Password"
        failed_return(root,frame1,message)
